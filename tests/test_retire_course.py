...
def test_setup_course_main_creates_directory(tmp_path: Path) -> None:
    """setup_course.main() copies template and creates destination directory."""
    from setup_course_github import setup_course
    ...
    with patch(
        "setup_course_github.setup_course.get_github_user", return_value=mock_user
    ):
...
def test_setup_course_main_with_name_suffix(tmp_path: Path) -> None:
    """setup_course.main() includes optional name suffix in destination."""
    from setup_course_github import setup_course

    mock_user = MagicMock()
    mock_user.create_repo.return_value = MagicMock()
